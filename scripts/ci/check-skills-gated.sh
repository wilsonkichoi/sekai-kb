...
SCRATCH_DIR="$ROOT/.claude/skills/__gate_selftest__"
SCRATCH="$SCRATCH_DIR/SKILL.md"
...
# 0xE4 0xB8 0xAD, written via octal) inside a scratch skill file.
mkdir -p "$SCRATCH_DIR"
{
...
# The genericity gate must now FAIL (place string under .claude/skills/).
if bash "$GEN_GATE" >/dev/null 2>&1; then
  echo "❌ skills-gate self-test: genericity gate did NOT catch '$PLANT_TERM' in .claude/skills/ (instance mode)" >&2
  exit 1
fi

# The english-only gate must now FAIL (CJK codepoint under .claude/skills/).
if node "$CJK_GATE" >/dev/null 2>&1; then
  echo "❌ skills-gate self-test: english-only gate did NOT catch a CJK codepoint in .claude/skills/ (instance mode)" >&2
  exit 1
fi

echo "✓ skills-gate self-test passed — both gates scan .claude/skills/ in instance mode ('$PLANT_TERM' + CJK caught)"
